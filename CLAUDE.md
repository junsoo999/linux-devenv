...
- 이 레포는 **Click 기반 Python CLI(`devenv`)와 dotfile 자산**으로 Linux 서버 / macOS 워크스테이션의 개발 환경(Z-Shell, Neovim, Tmux, Powerlevel10k 등)을 자동으로 구성한다.
...
3. **Linux / macOS target** — 이 레포는 Linux 서버 + macOS 워크스테이션을 지원한다. `devenv.cli._platform.ensure_supported()`로 진입을 가드한다. 패키지 매니저는 분기하라 — Linux는 `apt-get`/`apt`, macOS는 `brew`. `brew`를 가정한 코드는 `devenv.cli._platform.is_macos()` 체크 안에서만 쓰고, BSD `sed -i ''` 같은 OS 종속 옵션은 양쪽에서 동작하는 형태로 작성하라. Windows는 지원하지 않는다.
